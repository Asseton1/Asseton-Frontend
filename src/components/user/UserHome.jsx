...
// Match PropertyListing URL param handling: same price/sqft/cents ranges
...
function UserHome() {
  ...
  const [_listingTypeDirty, setListingTypeDirty] = useState(false);
  ...
  const handlePropertyTypeSelect = (type) => {
    ...
    const isLandProperty = propertyTypes.some(
      (propertyType) =>
      ...
      .then((permissionStatus) => {
        ...
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            // Reverse geocode to get address from coordinates (for display in input)
            reverseGeocode(lat, lng);

            // Automatically fetch and display properties by navigating to listing with lat/lng
            const params = buildListingParams({ lat, lng });
    ...
    const fetchProperties = async () => {
      try {
        ...
        if (propertiesData.length > 0) {
          const sortedPropertiesData = [...propertiesData].sort(
  ...
  // Build property-listing URL params from current filters; optional lat/lng override (e.g. from "Use my location")
  const buildListingParams = (coordsOverride) => {
    const params = new URLSearchParams();

    // type (buy/rent) — same as filter
    if (activeTab === "buy" || activeTab === "rent") {
      params.set("type", activeTab);
    }

    // search (location text) — only when not using coordinates
    const useCoords =
      (coordsOverride &&
    ...
    const coords = coordsOverride || userCoordinates;
    if (!useCoords && location && location.trim()) {
      params.set("search", location.trim());
    }

    // latitude, longitude — same as filter (property-listing uses these names)
    if (useCoords && coords) {
      params.set("latitude", String(coords.lat));
      params.set("longitude", String(coords.lng));
    }

    // propertyType (category) — same as filter; listing lowercases it
    if (selectedPropertyType && selectedPropertyType !== "All") {
      params.set("propertyType", selectedPropertyType);
    }

    // price_min, price_max — same as filter (numeric)
    if (priceRange && priceRange !== "Price (INR)") {
      const [priceMin, priceMax] = getPriceRangeFromString(priceRange);
      params.set("price_min", String(priceMin));
      params.set("price_max", String(priceMax));
    }

    // area: area_unit + area_min + area_max (same as filter) for Sq.ft or Cents
    if (propertyMetricValue && propertyMetricValue !== "Any") {
      if (propertyMetric === "Cents") {
        const [centsMin, centsMax] =
          getCentsRangeFromString(propertyMetricValue);
        params.set("area_unit", "cent");
        params.set("area_min", String(centsMin));
        params.set("area_max", String(centsMax));
      } else {
        // Sq.ft (or default when dropdown shows sq.ft options)
        const [sqftMin, sqftMax] =
          getSquareFeetRangeFromString(propertyMetricValue);
        params.set("area_unit", "sqft");
        params.set("area_min", String(sqftMin));
        params.set("area_max", String(sqftMax));
      }
    }
  ...
  // Handle search — use same query param names and formats as property-listing filter section
  const handleSearch = () => {
    const params = buildListingParams(null);
    navigate(`/property-listing?${params.toString()}`);
  };
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-grow">
        ...
        <header className="relative overflow-hidden h-[350px] xs:h-[400px] sm:h-[450px] md:h-[600px] lg:h-[650px] xl:h-[700px] mt-16 sm:mt-18 md:mt-20 md:mx-2 lg:mx-4 xl:mx-6 md:rounded-3xl">
          ...
          <div
            className="hidden"
            id="ad-placement-pixel"
            data-google-query-id="CK6MoJmRto0DFQWnZgIdC_cd3A"
          >
          ...
          <div className="absolute inset-0 w-full overflow-hidden md:rounded-3xl">
            {bannerLoading ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-green-600 via-green-500 to-emerald-600 md:rounded-3xl"></div>
            )}
          ...
          <div className="absolute inset-0 flex flex-col justify-center items-center px-4 z-10">
            <div className="text-center">
              ...
              <div className="hidden lg:block w-full max-w-4xl mx-auto">
                <div
                  className="w-full rounded-2xl shadow-lg p-5 lg:p-6 search-form"
                        ...
                        <div className="flex w-full">
                          {["Buy", "Rent"].map((tab) => (
                            <button
                              key={tab}
                              onClick={() =>
                                handleActiveTabChange(tab.toLowerCase())
                              }
        ...
        <section className="lg:hidden bg-white py-6 sm:py-8">
          <div className="container mx-auto px-4">
            <div
              className="w-full sm:w-11/12 md:w-4/5 lg:w-3/5 mx-auto rounded-xl shadow-lg p-3 sm:p-4 md:p-6 search-form"
              style={{ backgroundColor: "rgb(240 253 244)" }}
            >
              {/* Top Row: Buy/Rent Toggle, Property Type, Sq.ft, Price - All in one row */}
            ...
            <div className="max-w-7xl mx-auto overflow-hidden rounded-2xl shadow-xl bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 border border-gray-100">
              <div className="responsive-flex-row items-center">
                ...
                <div className="responsive-w-half p-6 sm:p-8 md:p-10 flex flex-col justify-center">
                  <h3 className="heading-3 text-gray-800 mb-3">
                    Exclusive Property Offer
                  </h3>
                  <p className="body-medium text-gray-600 mb-4 sm:mb-6 leading-relaxed">
                    Experience luxury living with our premium properties. Enjoy
                    special discounts and priority access for a limited time
                    only.
                  </p>
        ...
        <section className="bg-white">
          <div className="container mx-auto container-padding section-padding-sm">
            <div className="responsive-flex-row justify-between items-center mb-6 sm:mb-8">
              <h2 className="heading-3 text-gray-900 mb-4 md:mb-0">
                Featured Properties
              </h2>
            </div>
              ...
              <div className="relative overflow-hidden">
                ...
                >
                  {properties.map((property) => {
                    ...
                    const locationDisplay = [city, district, state]
                      .filter(Boolean)
                      .join(", ");
                        ...
                        >
                          ...
                          <div className="p-3 sm:p-4">
                            ...
                            <p className="text-gray-600 text-xs sm:text-sm">
                              {property.property_type_details?.name ||
                ...
                <div className="flex justify-center mt-8 space-x-2">
                  {Array.from({
                    length: Math.ceil(properties.length / itemsToShow),
                  }).map((_, index) => (
                    <button
                      key={index}
            ...
            <div className="responsive-grid-4 responsive-gap">
              ...
              <div className="group">
                <div className="bg-green-50 rounded-2xl p-4 sm:p-6 transition-all duration-300 group-hover:bg-green-100">
                  ...
                  <p className="body-small text-gray-600">
                    Over 1000+ satisfied clients and growing, with a proven
              ...
              <div className="group">
                <div className="bg-green-50 rounded-2xl p-4 sm:p-6 transition-all duration-300 group-hover:bg-green-100">
                  ...
                  <p className="body-small text-gray-600">
                    Quick response times and streamlined processes to save you
                    time and effort
                  </p>
                </div>
              </div>
              ...
              <div className="group">
                <div className="bg-green-50 rounded-2xl p-4 sm:p-6 transition-all duration-300 group-hover:bg-green-100">
                  ...
                  <p className="body-small text-gray-600">
                    Experienced professionals dedicated to helping you find your
                    perfect property
                  </p>
                </div>
              </div>
              ...
              <div className="group">
                <div className="bg-green-50 rounded-2xl p-4 sm:p-6 transition-all duration-300 group-hover:bg-green-100">
                  ...
                  <p className="body-small text-gray-600">
                    Transparent transactions and secure processes for your peace
                    of mind
                  </p>
                </div>
              </div>
        ...
        <section className="section-padding bg-gray-50">
          <div className="container mx-auto container-padding">
            <div className="responsive-text-center mb-8 sm:mb-12">
              ...
              <p className="body-medium text-gray-600 max-w-2xl mx-auto">
                Discover why our clients trust us with their dream properties
              </p>
            </div>
            ...
            <div className="responsive-grid-3 responsive-gap">
              ...
              <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md hover:shadow-xl transition-shadow relative">
                ...
                <div className="pt-4 sm:pt-6">
                  <p className="body-small text-gray-600 mb-3 sm:mb-4">
                    "Finding our dream home was effortless with their
              ...
              <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md hover:shadow-xl transition-shadow relative">
                ...
                <div className="pt-4 sm:pt-6">
                  <p className="body-small text-gray-600 mb-3 sm:mb-4">
                    "The property listings were accurate, and the team's
                    expertise in the local market made our investment decision
                    easy."
                  </p>
                  <div className="flex items-center">
                    <div>
              ...
              <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-md hover:shadow-xl transition-shadow relative">
                ...
                <div className="pt-4 sm:pt-6">
                  <p className="body-small text-gray-600 mb-3 sm:mb-4">
                    "Professional, transparent, and truly customer-focused. They
                    made our property search journey smooth and genuinely
                    enjoyable."
                  </p>
                  <div className="flex items-center">
                    <div>
        ...
        <section className="section-padding bg-gradient-to-r from-gray-800 to-gray-800 relative overflow-hidden">
          <div className="absolute inset-0">
            ...
            >
              ...
              <defs>
                ...
                >
                  <path
                    d="M-3 3l6-6M7 13l6-6M-3 13l6-6M7 3l6-6"
                    stroke="currentColor"
                    strokeWidth="1.5"
                  />
                </pattern>
          ...
          <div className="container mx-auto container-padding relative">
            <div className="max-w-3xl mx-auto responsive-text-center">
              ...
              <p className="body-medium text-green-50 mb-6 sm:mb-8">
